...
/**
 * PUT /api/animations/[id]/archive
 * Archive an existing animation
...
export async function PUT(
  ...
  try {
    ...
    if (!user) {
      logger.warn({ path: '/api/animations/[id]/archive', method: 'PUT' }, 'Unauthorized access attempt')
    ...
    // Archive animation (service handles ownership check, admin bypasses)
    const animation = await animationService.archiveAnimation(animationId, user.userId, user.role)
    ...
    logger.error(
      ...
      'Error archiving animation'
    )
    return errorResponse('INTERNAL_3000', 'Une erreur est survenue lors de l\'archivage', 500)
  }
}
